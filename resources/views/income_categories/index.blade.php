...
@section('title', 'Categorias de Renda')
                    ...
                    @foreach ($incomeCategories as $category)
                        <tr
                            ...
                            <td class="px-6 py-4 flex justify-center space-x-4">
                                <a href="{{ route('income_categories.edit', $category->id) }}"
                                    class="text-blue-600 dark:text-blue-500 hover:underline">{{ __('Editar') }}</a>
                                <form action="{{ route('income_categories.destroy', $category->id) }}" method="POST"
                                    class="inline">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit"
